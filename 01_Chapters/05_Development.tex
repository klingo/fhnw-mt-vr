...
Since the different views are rather far away from where the user is spawned, a pointer functionality has been introduced to the gesture controller. This is shown in Figure XXX alongside the button tutorial of the controller which is shown for a limited time in the form of tooltips.

TODO: Create and add figure
\begin{figure}[h]
	\begin{center}
		\includegraphics[width=14cm]{03_Figures/08_Development/Unity_Pointer_Tutorial.png}
		\caption{Gesture Controller Pointer and Button Tutorial as Tooltips (live in \gls{vr})}
		\label{fig:unitypointer}
	\end{center}
\end{figure}

By referring back to the Interaction Map in Figure \ref{fig:interactionmap}, four main interactions can be distinguished, plus a fifth one for reading in the data from a CSV file.
\begin{itemize}[noitemsep,nolistsep]
