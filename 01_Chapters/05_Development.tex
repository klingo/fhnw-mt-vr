...
\end{itemize}
By looking at \gls{mdg} 2 that defines the user to be part of the \gls{ve} and thus is able to 'travel' around the visualisations, the third option would violate this design goal and thus is no viable option. Comparing the Full Body Tracking with the Motion Tracking, the first one lacks accuracy and only works from one angle whereas the Motion Tracking works in 360° and is also very accurate. Therefore, only the '360° Motion Tracking' passes all relevant design goals.

For \textit{Selection} (and \textit{Manipulation}) the some more methods had been discussed:
\begin{itemize}[noitemsep,nolistsep]
	\item Hand Gestures
	\item Gesture Controllers
	\item Speech Recognition
	\item Physical Placement of Objects
	\item No input option
\end{itemize}
