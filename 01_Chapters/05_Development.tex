...
\begin{enumerate}[noitemsep,nolistsep]
	\item When clicking on a bar, the \texttt{ChartBarClick.cs} calls the \texttt{updateSelection()} method
	\item This first resets the DetailView with \texttt{updateDetailView(null);}
	\item Then it starts a Coroutine (i.e. flow control is cooperatively passed between it and the game engine)
	\item The Coroutine opens a 2nd Thread for the CPU intensive work not not block the GameEngine
	\item The 2nd Thread filters all data, updates the ArrayLists and caches the results
	\item Meanwhile, Coroutine checks if Thread is still running and gives temporary flow control to GameEngine
