...
Since the filtering for the categories is applied on the selected data (compared to the month selection that 'resets' the selected data), it should be always in front of the user for easy access. While this could be done in some kind of pop-up menu or similar, \cite{Bowman2002} reminds us that these techniques are far less natural and could require some training for the user, especially when it is not clear that such a pop-up menu exists and how it can be displayed. Instead, and also in accordance with \gls{mdg} 3, the filtering is intended for \gls{vr} and thus should also be done in 3D. The main categories that are listed in Table \ref{tbl:financialcategories} already have some icons assigned to them in the UBS e-Banking, and are shown more detail in Figure \ref{fig:categoriesicons}.
\begin{figure}[h]
	\begin{center}
		\includegraphics[width=7cm]{03_Figures/07_Suggestion/UBSAG2016_CategoriesIcons.png}
		\caption[Mapping of icons to expense categories in UBS e-banking demo]{Mapping of icons to expense categories in UBS e-banking demo \citep{UBSAG2016}}
