...
The \textit{Selection} pattern is undeniably the most important one since the core activity is data exploration. For a better understanding, Figure \ref{fig:interactionmap} gives an overview of all views and their interaction paths that have been numbered.
\begin{figure}[h]
	\begin{center}
...
Similar to Path 4, it is also possible to select the 'bar' of a single day to trigger an update on the list of financial transactions. This path can also be indirectly triggered when a month is selected in the Year Overview.
...
\textbf{View 3: Month/Year Selection} \newline
Starting from this first view, the user can select a year and/or a month which will trigger a data update on the Year Overview (Path 1), the Month Overview (Path 2), and the list of Financial Transactions (Path 3). If only a year was selected, then the Month Overview will remain empty and the \newline

\textbf{View 4: Categories Selection} \newline
