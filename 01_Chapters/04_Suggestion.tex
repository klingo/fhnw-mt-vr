...
\end{itemize}
In addition to the above characteristics, another goal has to be that the Design should not rely on very specific \gls{vr}-hardware, but rather define them in a more abstract way as to what the hardware item should be capable of to be a feasible option for the Design.
\begin{itemize}[noitemsep,nolistsep]
	\item \textbf{\gls{mdg} 4:} \gls{vr}-hardware independent design \newline
	\textit{Example: No direct references to specific hardware, but rather what the hardware item has to be capable of}
\end{itemize}
