...
As for travel, \cite{Bowman2002} defined guidelines for designing selection techniques that can be summarised as follow:
\begin{itemize}[noitemsep,nolistsep]
...
\begin{figure}[h]
	...
	\end{center}
\end{figure} \newline
In their game \textit{The Gallery - Episode 1: Call of the Starseed}, \cite{CloudheadGames2016} provide the option to select objects by utilizing the gesture controllers that in \gls{vr} look like hands as shown in Figure \ref{fig:theGalleryHands}. With the trigger button, the hand can be closed and thus objects can be grabbed and moved around. The game further distinguishes between "important" and "unimportant" objects, where the "important" ones are stuck in the grip of the hand even if the trigger was release (i.e. a flashlight) and "unimportant" objects would immediately drop once the trigger button is released. This allows for a more convenient interaction since it can become irritating to keep the trigger button held for a longer amount of time. \newline
\begin{figure}[h]
	\begin{center}
...
The research of \cite{Bowman2002} has shown, that although this type of non-natural interaction indeed can be quite effective, especially since it allows for tasks to be executed that cannot be done naturally. Although it would be possible to provide a virtual colour palette where the user would have to choose the colour to eliminate this option from the pop-up menu, this would drastically impact the task performance.
\begin{figure}[h]
	\begin{center}
...
Building up on the research done in chapter \ref{SectionLiteratureReviewSRQ1} and \ref{SectionLiteratureReviewSRQ2}, this  chapter focuses on combinations and enhancements of interaction patterns and input methods, thus addressing the third \gls{srq}.
\begin{framed}
...
Under the aspect of immersion and task performance, the interaction methods with gesture controllers and 360° motion tracking showed to be the most promising. The 360° motion tracking has just been released recently and not much research has been conducted on it yet, while the gesture controllers provide almost the same similarity like hand gestures, but are far more accurate and are well integrated with the motion tracking. In terms of interaction patterns, the \textit{Travel} can in its simplest form become irrelevant due to the natural movement that is possible with the 360° motion tracking. The focus therefore lies on the patterns for \textit{Selection} and especially \textit{Manipulation}.
...
\subsection{To be defined}

The third and final \gls{srq} focuses on the selection and enhancement of existing interactions patterns with new methods for user input which will be implemented, tested and evaluated in a \gls{vr} application prototype. It therefore is not discussed as part of the Master Thesis Research Proposal.

% "The fourth and final sub research question aims at selecting one or more object recognition methods which will be tested and evaluated in a robot prototype"
...
To be completed as part of the main Master Thesis.
