...
Generally it can be said though that only little research is focussing on the utilization of 2D objects for the 3D space. \cite{Ware2012} even see clear advantages in the conventional two-dimensional techniques such as the bar chart or the scatter plot, especially since our most powerful pattern-finding mechanism in the brain does in fact work in 2D and not 3D. Furthermore, there are well established and effective (2D) data representations that are also very easy to be included in any kind of two-dimensional medium such as a book or a report \citep{Ware2012}. On the other side, \cite{Ware2012} also point out that we live in a three-dimensional word and interact with it so frequently that our brains got very much used to it. It is also important to understand that ultimately a 2D space is part of 3D space and thus an 2D object can always be flattened out and presented as such in 3D space. \cite{Kwon2015} also said that in a 2D environment/space (like a regular screen) it makes sense to use 2D objects for the presentation, whereas in a \gls{vr} environment where the viewing direction can be freely chosen by the user, the whole three dimensional space should be utilized.
\begin{figure}[h]
	\begin{center}
...
In terms of interacting with the visualisations, it is interesting to see that none of the before discussed applications actually implemented any "zoom" in the way it is known from 2D applications, but rather allow the user to freely navigate around in the \gls{ve} and get as close to the data as desired.
