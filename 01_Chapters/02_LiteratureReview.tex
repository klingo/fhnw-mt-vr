...
When we talk about the presentation of data, many will immediately think of Microsoft Excel and the many different charts (e.g. bar chart, line chart, pie chart, scatter plot etc.) that can be created with just a few clicks based on almost any data set. But not only in such mass consumer products, also big \gls{bi} solutions like \cite{TableauSoftware2016} rely on these well known and widely accepted techniques for their visualization of (big) data. As a potential solution fo this problem in \gls{vr}, \cite{Hentschel2009} come up with the concept of 'multiple linked views', whose principle it is that several different views are presented to the user which all are linked together in the background and make sure that the displayed data is always in sync on all these views. Although this concept has shown to be a good solution for interactive analysis of time-dependent data, one major short-coming when limited to a standard desktop-based setup is that complex spatial relationships are hard to understand when the data is only a 2D projection \citep{Hentschel2009}. \newline
